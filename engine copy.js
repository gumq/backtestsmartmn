...
const { validateSignal } = require("./signalValidator");
...
// ---- ORDER FLOW ----
const ABSORPTION_DELTA_USD = TEST_MODE ? 40_000 : 300_000;
...
function detectAbsorption(symbol) {
  ...
  const durationMs =
    trades.at(-1).time - trades[0].time;
...
function mtfFilter(symbol) {
  ...
  if (!TEST_MODE) {
    ...
    if (p1h === "Markdown") return null;
    if (["Markdown", "Phase E"].includes(p15m)) return null;
    if (!["Phase B", "Phase C", "Phase D","Transition"].includes(p1m)) return null;
  }
...
function getRisk(symbol, phase, vp) {
  ...
  // LIVE / NORMAL MODE
  // ============================
  if (!vp && TEST_MODE === false) return null;
...
//   if (!validateSignal(signal, state)) return;
...
//   if (risk.rr < minRR) {
//     console.log(`[SKIP][RR] ${symbol.toUpperCase()} rr=${risk.rr.toFixed(2)}`);
//     return;
//   }
...
//   // ===== REGIME-BASED FILTER =====
//   const minPwin =
//     regime === "RISK_OFF" ? Number(process.env.RISK_OFF_MIN_PWIN) : 0.75;
...
//   // ===== TELEGRAM / LOG =====
//   // ==================================================
//   const msg = `🚨 AUTO EXECUTED (A+B+C+D+F+G) MODE ${
//     TEST_MODE === true ? "Test" : "Live"
//   }
...
async function detect(symbol) {
  ...
  // ===== BUILD SIGNAL (ALPHA ONLY) =====
  const signal = buildSignal(symbol, state);
  if (!signal) return;

  // ===== VALIDATE SIGNAL (RISK + AI + REGIME) =====
  if (!validateSignal(signal, state)) return;

  // ===== PROCESS SIGNAL (PORTFOLIO + EXECUTION + LOG) =====
  await processSignal(signal);
}
